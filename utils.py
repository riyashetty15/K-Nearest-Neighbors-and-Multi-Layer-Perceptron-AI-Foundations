...
def tanh(x, derivative = False):
    ...
    Args:
        ...
        derivative: A boolean representing whether or not the derivative of the function should be returned instead.
    """
    t_num = np.exp(x) - np.exp(-x)
    t_den = np.exp(x) + np.exp(-x)
    th = t_num / t_den
    if not derivative:
        return th
    else:
...
def one_hot_encoding(y):
    ...
    y_ohe = np.array([])
    for yi in y:
        y_row = np.zeros(len(y_classes))
        x = np.where(y_classes == yi)
        y_row[x] = 1
        y_ohe = np.append(y_ohe, y_row)
    return y_ohe.reshape(y.shape[0], y_classes.shape[0])
